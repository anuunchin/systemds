...
/**
 * Rule: RewriteFloorCompress. Detects the sequence `M2 = floor(M * S)` followed by 
 * `C = compress(M2)` and prepares for fusion into a single operation. 
 * This rewrite improves performance by avoiding intermediate results.
 * Currently, it identifies the pattern without applying fusion.
 */
public class RewriteFloorCompress extends HopRewriteRule
{
    @Override
